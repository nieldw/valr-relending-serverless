...
import {CurrencyInfo, OpenLoan, Subaccount, SubaccountBalance, UpdateLoanRequest, ValrCredentials} from '../types/valr';
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (error.message?.includes('Rate limit exceeded') && (error as any).rateLimitInfo && attempt < maxRetries) {
                    ...
                    console.warn(`Rate limited. Waiting ${waitTimeMs}ms before retry ${attempt + 1}/${maxRetries}`, {
                        endpoint,
                        waitTimeMs
                    });
